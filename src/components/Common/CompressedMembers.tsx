    ...
    return <div className={"flex " + className}> {
        members.slice(0, 9).map((student, index, array) =>
            <div className="-mr-1.5 relative" style={{ zIndex: 100 - index }} key={index}>
                {array.length - 1 == index && <div className="rounded-full bg-black bg-opacity-50 absolute w-full h-full z-10 text-gray-300 items-center justify-center flex text-xs border-gray-100 border-2">
