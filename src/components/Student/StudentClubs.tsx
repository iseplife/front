        ...
        {clubs ?
            clubs.length > 0 ? clubs.map(club => 
                <Link to={`/club/${club.club.id}`} className="mx-auto">
                    <div
                        className="mx-1 bg-white rounded-2xl overflow-hidden hover:shadow-sm transition-shadow h-48 px-4 items-end flex aspect-[18/20]"
                        style={{
                    ...
                    >
                        <div className="bg-white/40 rounded-xl backdrop-blur grid place-items-center px-3 py-1 mb-3 mt-auto w-full text-neutral-800 font-semibold text-2xl">
                            ...
                            <span className="text-sm font-bold text-neutral-600">
                                {club.position} <FontAwesomeIcon className="ml-2" icon={ClubRoleIcon[club.role]}/>
                            </span>
                        </div>
                    </div>
                </Link>
