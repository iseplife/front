...
const SubscriptionExtensiveButton: React.FC<SubscriptionExtensiveButtonProps> = ({ id, type, extensive, updateExtensive }) => {
    ...
    return <div className="w-12 grid place-items-center relative ml-0 mr-auto h-10">
        <FontAwesomeIcon onClick={handleExtensive} icon={extensive ? faSolidBell : faBell} className="text-indigo-400 text-2xl cursor-pointer hover:text-opacity-90" />
        {extensive && <FontAwesomeIcon icon={faCheck} className="top-1.5 right-[3px] pointer-events-none text-indigo-400 cursor-pointer text-sm absolute transform scale-75" />}
    </div>
}
