...
const EventPreviewList: React.FC<EventPreviewListProps> = ({ events }) => {
    ...
    return (
        <div className="grid rounded-lg overflow-hidden relative sm:mb-4">
            ...
            <div onClick={scrollRight} className={"sm:hidden cursor-pointer absolute right-0 bg-black/[12%] backdrop-blur-sm grid place-items-center w-6 h-full text-lg transition-opacity " + (maxScroll <= 0 || scroll > maxScroll - 10 && "opacity-0 pointer-events-none")}>
