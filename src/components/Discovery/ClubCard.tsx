...
const ClubCard: React.FC<ClubCardProps> = ({club}) => {
    const getLogo = (club:ClubPreview) => {
        if(!club?.logoUrl || club.logoUrl.split("/")[1] === "clb"){
            return mediaPath(club.logoUrl, AvatarSizes.FULL)
        }
        return club.logoUrl
    }
    return (
        <LinkEntityPreloader noWidth preview={club}>
            <Link to={`/club/${club.id}`}>
                <div
                    ...
                    style={{
                        backgroundImage: `url("${getLogo(club)}")`,
                        backgroundRepeat: "no-repeat",
                        backgroundPosition: "center",
                    }}
                >
                    <div
                        className="bg-white/40 text-center rounded-xl backdrop-blur grid place-items-center px-3 py-1 mb-3 mt-auto w-full text-neutral-800 font-semibold text-2xl">
                        {club.name}
                    </div>
                </div>
            </Link>
